...
| dosen pengampu (`dosentugas.prodi_kode`) | masuk daftar pilihan pengajar kelas prodi itu (sejak 2026-09-25 centang ini saja tidak memberi hak menulis) |
...
| kaprodi | + kurikulum, kalender, centang pengampu, kelola kelas (pengajar, peserta, tambah/hapus), semua hak pengajar di kelas prodinya, laporan prodinya, halaman Pengguna untuk prodinya (tambah dosen, reset kata sandi mahasiswa prodinya dan dosen pengampunya) |
...
## Alur tipikal
- Kaprodi menyiapkan prodi: `PUT /kurikulum/prodi/{prodi}` → rumpun/CPL → `PUT /kurikulum/ritme/{nama}` → `POST /kalender` → `PUT /kalender/{id}` → `POST /kalender/{id}/terbitkan` (kelas dibuat otomatis) → `PUT /jabatan/dosen/{email}/prodi` (daftar pengampu) → `GET /kelas/pilihan-pengajar` → `PUT /kelas/{id}` (tunjuk pengajar).
- Kelas: `GET /kelas/saya` → `GET /kelas/{id}` → `GET /kelas/{id}/isi` → `GET /kelas/{id}/nilai`.
- Tugas kelas: `POST /kelas/{id}/tugas` (+`POST /tugas/{id}/lampiran`) → mahasiswa `GET /tugas/{id}` → `POST /tugas/{id}/kirim` → pengajar `GET /tugas/{id}/kiriman` → `GET /tugas/kiriman/{kid}/berkas` → `PUT /tugas/{id}/kiriman/{kid}/nilai` → `GET /tugas/{id}/laporan`. Tugas prodi lama: `POST /tugas` (wajib `prodi_kode`, pengajar kelas di prodi itu/kaprodi).
