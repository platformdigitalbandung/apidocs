...
| dosen | baca semua prodi; roster & email kampus; tinjau RPL, rapor per NIM, ujian, proyek kerja bimbingan |
| pengajar kelas (email di `kelas.pengajar`, ditunjuk kaprodi) | + materi & kuis rumpun/mata kuliah kelas itu, tugas bertenggat, penilaian kiriman, bobot & buku nilai, rekaman sesi kalender kelas itu |
| kaprodi | + kurikulum, kalender, kelola kelas (pengajar dari semua dosen aktif, peserta, tambah/hapus), semua hak pengajar di kelas prodinya, laporan prodinya, halaman Pengguna untuk prodinya (tambah dosen, reset kata sandi mahasiswa prodinya dan dosen yang tercatat di prodinya) |
...
| 2 · Tulis yang mudah diperbaiki | `POST /materi`, `PUT /materi/{id}`, `POST /materi/{id}/berkas`, `POST /mahasiswa` (NIM baru), `POST /kalender`, `PUT /kalender/{id}` (draf), `POST /kurikulum/prodi/{prodi}/rumpun`, `POST /kurikulum/prodi/{prodi}/cpl`, `POST /kurikulum/ritme`, `PUT /kurikulum/prodi/{prodi}/matakuliah/{kode}`, `POST /kuisgerbang` (kuis baru), `POST /sesiujian`, `PUT /autograder/bobot`, `POST /kelas/{id}/tugas`, `PUT /tugas/{id}`, `POST /tugas/{id}/lampiran`, `POST /kelas`, `POST /kelas/otomatis`, `PUT /tugas/{id}/kiriman/{kid}/nilai` dengan `kembalikan: false` (draf nilai), `POST /kelas/{id}/pengumuman` dengan `kirim_grup: false` | Tampilkan ringkasan rencana (berapa aksi, rute apa, isian pokok), tunggu satu kali **"ya"** untuk seluruh kumpulan itu. |
| 3 · Berdampak, menimpa, atau sulit dibatalkan | semua `DELETE`; `POST /kalender/{id}/terbitkan`; `POST /proyekblok`, `POST /proyekblok/{id}/nilai` (dipensiunkan, jangan dipakai untuk nilai baru); `POST /proyekkerja/{id}/putusan`, `POST /proyekkerja/{id}/tinjauan`, `POST /proyekkerja/{id}/tautan-tinjauan`; `POST /rpl/{id}/tinjau`; `POST /rekaman`; `POST /sesiujian/{id}/checkin`; `PUT /kelas/{id}` (pengajar/peserta); `PUT /kelas/{id}/bobot`; `PUT /kelas/{id}/nilai/{nim}`; `PUT /tugas/{id}/kiriman/{kid}/nilai` dengan `kembalikan: true`; `POST /kelas/{id}/pengumuman` dengan `kirim_grup: true` (terkirim ke grup WhatsApp, tidak bisa ditarik); `PUT /kurikulum/prodi/{prodi}/kaprodi`; `POST /kurikulum/prodi`; `POST /kurikulum/seed`; `PUT /dosen/email`; `POST /mahasiswa` untuk NIM yang **sudah ada**; `POST /pengguna/dosen`, `PUT /pengguna/dosen/{id}`, `PUT /pengguna/dosen/{id}/aktif`, `PUT /pengguna/dosen/{id}/admin`, `POST /pengguna/sandi/reset`, `PUT /akun/sandi`; `POST /kuisgerbang` yang **mengganti** kuis yang ada; `POST /tugas/{id}/kirim`, `POST /kuisgerbang/{id}/submit`, `POST /proyekkerja`, `POST /rpl` (atas nama mahasiswa) | Konfirmasi **per aksi**: tampilkan metode, path, body persis, dan dampaknya (untuk yang menimpa: nilai lama → nilai baru, dari `GET` sebelumnya), lalu tunggu **"ya"** eksplisit untuk aksi itu saja. |
...
## Alur tipikal
- Kaprodi menyiapkan prodi: `PUT /kurikulum/prodi/{prodi}` → rumpun/CPL → `PUT /kurikulum/ritme/{nama}` → `POST /kalender` → `PUT /kalender/{id}` → `POST /kalender/{id}/terbitkan` (kelas dibuat otomatis) → `GET /kelas/pilihan-pengajar` (semua dosen aktif ber-email) → `PUT /kelas/{id}` (tunjuk pengajar).
- Kelas: `GET /kelas/saya` → `GET /kelas/{id}` → `GET /kelas/{id}/isi` → `GET /kelas/{id}/nilai`.
- Tugas kelas: `POST /kelas/{id}/tugas` (+`POST /tugas/{id}/lampiran`) → mahasiswa `GET /tugas/{id}` → `POST /tugas/{id}/kirim` → pengajar `GET /tugas/{id}/kiriman` → `GET /tugas/kiriman/{kid}/berkas` → `PUT /tugas/{id}/kiriman/{kid}/nilai` → `GET /tugas/{id}/laporan`. Semua tugas milik kelas (tugas prodi dihapus 2026-09-26).
